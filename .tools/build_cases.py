...
CASES.append({
    ...
    "sections": [
        ...
        ("진단·결정",
         ...
         "<p>다학제 회의에서 외과 절제와 고주파열치료술 두 옵션을 검토하였습니다. 종양 위치(피막에서 약간 깊음)와 간 기능 보존을 고려해 "
...
CASES.append({
    ...
    "sections": [
        ...
        ("진단·결정",
         ...
         "다학제 회의에서 절제·소작·이식을 모두 검토하였고, 종양이 피막 가까이에 있어 절제와 소작이 모두 가능하였습니다. "
...
CASES.append({
    ...
    "sections": [
        ...
        ("진단·결정",
         ...
         "0.8 cm로 작지만 진단 기준을 충족합니다. 다학제 회의에서 <strong>고주파열치료술</strong>을 결정하였습니다. "
...
CASES.append({
    ...
    "sections": [
        ...
        ("진단·결정",
         "<p>LR-4는 단독으로 진단 확정이 어려워 다학제 회의에서 결정하였습니다. 옵션은 1) 짧은 간격 추적(2-3개월), 2) 생검, "
...
CASES.append({
    ...
    "sections": [
        ...
        ("진단·결정",
         "<p>BCLC A기 단일 2.5 cm 간세포암으로 진단하였습니다. Child-Pugh A이며 다학제 회의에서 절제와 소작이 모두 가능하다고 평가되었습니다. "
...
CASES.append({
    ...
    "sections": [
        ("결정 흐름",
         ...
         "다학제 회의에서 <strong>같은 회기에 3개 결절을 모두 고주파열치료술</strong>로 시행하기로 결정하였습니다.</p>"),
        ("시술과 경과",
...
CASES.append({
    ...
    "sections": [
        ("평가",
         ...
         "알코올 1년 금주를 유지 중이라 이식 평가가 가능한 조건입니다. 다학제 회의에서 <strong>간이식 평가 시작</strong>을 결정하였습니다. "
...
CASES.append({
    ...
    "sections": [
        ("결정",
         ...
         "다학제 회의에서 <strong>conventional TACE</strong>를 1차로 결정하였습니다.</p>"),
        ("시술과 경과",
...
CASES.append({
    ...
    "intro": (
        ...
        "절제를 시행하기에는 잔여 간 부피 부족 위험이 있어 다학제 회의에서 결합 치료 전략을 결정하셨습니다.</p>"
    ),
    "sections": [
        ("결정",
         "<p>8 cm 단일 큰 종양은 절제·이식·TACE 단독 모두 한계가 있습니다. "
         "다학제 회의에서 <strong>TACE → 4-6주 후 SBRT</strong> 결합을 결정하였습니다. "
        ...
        ("시술과 경과",
         ...
         "다학제 회의에서 <strong>다운스테이징 성공으로 이식 평가</strong>를 시작하였습니다.</p>"),
    ],
    "teaching": [
...
CASES.append({
    ...
    "sections": [
        ("결정",
         ...
         "다학제 회의에서 <strong>atezolizumab + bevacizumab</strong> 1차 치료를 결정하였습니다.</p>"),
        ("치료와 경과",
...
CASES.append({
    ...
    "sections": [
        ("결정",
         ...
         "다학제 회의에서 <strong>STRIDE(durvalumab + tremelimumab 1회 priming)</strong>를 결정하였습니다. HIMALAYA 시험이 근거입니다.</p>"),
        ("치료와 경과",
...
CASES.append({
    ...
    "sections": [
        ("다운스테이징 시도",
         "<p>다학제 회의에서 <strong>TACE → SBRT → 6개월 안정 평가 후 이식 등록</strong> 흐름을 결정하였습니다. AFP 시작값은 1500이었습니다.</p>"
...
CASES.append({
    ...
    "sections": [
        ...
        ("결정과 경과",
         "<p>다학제 회의에서 위치(좌엽 표면), 크기(1.5 cm), 간 기능(Child-Pugh A)을 평가하여 "
